namespace DataAcquisition
{
    ...
    public static class ModbusUtility
    {
        ...
        public static byte[] CalculateCrc(byte[] array, int length)
        {
            if (array == null)
            {
        ...
        public static byte[] GetWriteSingleRegisterFrame(byte address, ushort registerAddress,ushort data)
        {
            byte[] frame = new byte[8];
            byte[] tempArray = new byte[6];

            byte[] tempAddr = BitConverter.GetBytes(registerAddress);
            ...
            Array.Copy(frame, tempArray, 6);

            byte[] crc = CalculateCrc(tempArray);
        ...
        public static byte[] GetReadHoldingRegisterFrame(byte address, ushort registerAddress, ushort numOfPoints)
        {
            byte[] frame = new byte[8];
            byte[] tempArray = new byte[6];

            byte[] tempAddr = BitConverter.GetBytes(registerAddress);
            byte[] tempData = BitConverter.GetBytes(numOfPoints);
        ...
        public static byte[] GetReadInclinationFrame(byte address)
        {
            byte cmd = 0x04;
